...
api.interceptors.request.use((config) => {
  ...
  try {
    currentUserId = JSON.parse(
      localStorage.getItem("quizflex_current_user") || "{}",
    )?.id;
  } catch {
    currentUserId = null;
  }

  const mockUserId = currentUserId || localStorage.getItem("mock_user_id");
...
export const roomsApi = {
  ...
  async updateCode(id, code) {
    const { data } = await api.patch(`/rooms/${id}/code`, { code });
    return unwrap(data);
  },
...
export const homeworkAssignmentsApi = {
  ...
  async getMySubmission(roomId, assignmentId) {
    const { data } = await api.get(
      `/rooms/${roomId}/assignments/${assignmentId}/my-submission`,
    );
    return unwrap(data);
  },
  ...
  async getSubmissionResult(roomId, assignmentId, submissionId) {
    const { data } = await api.get(
      `/rooms/${roomId}/assignments/${assignmentId}/submissions/${submissionId}`,
    );
    return unwrap(data);
  },
  ...
  async start(roomId, assignmentId) {
    const { data } = await api.post(
      `/rooms/${roomId}/assignments/${assignmentId}/start`,
    );
    return unwrap(data);
  },
  ...
  async answer(submissionId, payload) {
    const { data } = await api.post(
      `/assignment-submissions/${submissionId}/answer`,
      payload,
    );
    return unwrap(data);
  },
  ...
  async getSubmission(submissionId) {
    const { data } = await api.get(`/assignment-submissions/${submissionId}`);
    return unwrap(data);
  },
...
export const liveSessionsApi = {
  async currentForRoom(roomId) {
    const { data } = await api.get(`/rooms/${roomId}/live-sessions/current`);
    return unwrap(data);
  },

  async create(roomId, payload) {
    const { data } = await api.post(`/rooms/${roomId}/live-sessions`, payload);
    return unwrap(data);
  },
  ...
  async start(sessionId) {
    const { data } = await api.post(`/live-sessions/${sessionId}/start`);
    return unwrap(data);
  },

  async currentQuestion(sessionId) {
    const { data } = await api.get(
      `/live-sessions/${sessionId}/current-question`,
    );
    return unwrap(data);
  },

  async answer(sessionId, payload) {
    const { data } = await api.post(
  ...
  async nextQuestion(sessionId) {
    const { data } = await api.post(
      `/live-sessions/${sessionId}/next-question`,
    );
    return unwrap(data);
  },

  async leaderboard(sessionId) {
    const { data } = await api.get(`/live-sessions/${sessionId}/leaderboard`);
    return unwrap(data);
  },
};
...
export const normalizeAssignment = (assignment) => ({
  ...assignment,
  quizTitle:
    assignment.quiz_title || assignment.quiz?.title || "Quiz chưa có tên",
  quizDescription: assignment.quiz?.description || "",
  questionCount:
    assignment.quiz?.questions_count ??
  ...
  durationLabel: assignment.duration_minutes
    ...
    : "Không giới hạn",
  maxAttemptsLabel: assignment.max_attempts
