<template>
  <section class="grid min-h-[620px] items-center gap-12 py-10 lg:grid-cols-[1.05fr_0.95fr]">
    <div>
      <div class="mb-6 inline-flex rounded-full border border-[var(--border-strong)] bg-[var(--chip-active)] px-4 py-2 text-sm font-black text-[var(--primary)]">{{ $t('user_views.Home.hero_badge') }}</div>
      <h1 class="text-5xl font-black leading-[0.92] tracking-[-0.08em] text-[var(--text)] sm:text-6xl">
        {{ $t('user_views.Home.hero_title_prefix') }} <span class="gradient-text">{{ $t('user_views.Home.hero_title_highlight') }}</span>
      </h1>
      <p class="mt-7 max-w-2xl text-lg font-medium leading-8 text-[var(--muted)]">{{ $t('user_views.Home.hero_description') }}</p>
      <div class="mt-8 flex flex-wrap gap-3">
        <router-link class="btn-primary" :to="firstQuizLink">{{ $t('user_views.Home.try_quiz_button') }}</router-link>
        <router-link class="btn-ghost" to="/join-room">{{ $t('user_views.Home.join_room_button') }}</router-link>
        <router-link class="btn-ghost" :to="dashboardLink">{{ $t('user_views.Home.dashboard_button') }}</router-link>
      </div>
    ...
    <div class="relative">
      ...
      <div class="relative overflow-hidden rounded-[2rem] border border-[var(--border)] bg-[var(--surface)] shadow-[var(--shadow-soft)] backdrop-blur-2xl">
        <div class="flex h-12 items-center gap-2 border-b border-[var(--border)] bg-[var(--surface-soft)] px-5"><span class="h-3 w-3 rounded-full bg-[#ff5f57]"></span><span class="h-3 w-3 rounded-full bg-[#ffbd2e]"></span><span class="h-3 w-3 rounded-full bg-[#28c840]"></span><span class="ml-2 text-xs font-black text-[var(--muted)]">{{ $t('user_views.Home.live_preview') }}</span></div>
        <div class="grid gap-4 p-5">
          <div class="rounded-3xl border border-[var(--border)] bg-[var(--surface-soft)] p-5">
            <div class="flex items-start justify-between gap-5">
              <div><p class="text-sm font-black text-[var(--primary)]">{{ $t('user_views.Home.current_quiz_label') }}</p><h3 class="mt-2 text-xl font-black text-[var(--text)]">{{ featuredQuiz?.title || $t('user_views.Home.no_quiz_title') }}</h3><p class="mt-2 text-sm leading-6 text-[var(--muted)]">{{ featuredQuiz ? $t('user_views.Home.featured_quiz_meta', { questions: featuredQuiz.questions, duration: featuredQuiz.duration, difficulty: featuredQuiz.difficulty }) : $t('user_views.Home.no_quiz_description') }}</p></div>
          ...
          <div class="grid gap-3">
            ...
            <div v-if="previewQuizzes.length === 0" class="rounded-2xl border border-[var(--border)] bg-[var(--surface-soft)] p-3 text-sm font-bold text-[var(--muted)]">{{ $t('user_views.Home.no_public_quiz') }}</div>
          </div>
        </div>
      </div>
    </div>
  ...
  <section id="quiz-topics" class="relative py-12">
    ...
    <div class="relative z-10 mb-8 flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
      <div><p class="font-black text-[var(--primary)]">{{ $t('user_views.Home.available_quiz_label') }}</p><h2 class="mt-3 text-4xl font-black tracking-[-0.06em] text-[var(--text)] lg:text-5xl">{{ $t('user_views.Home.topics_title') }}</h2><p class="mt-4 max-w-2xl leading-7 text-[var(--muted)]">{{ $t('user_views.Home.topics_description') }}</p></div>
      <div class="group flex w-full max-w-md items-center gap-3 rounded-full border border-[var(--border)] bg-[var(--surface)] px-4 py-3 shadow-[var(--shadow-card)] backdrop-blur-xl transition focus-within:border-[var(--border-strong)]"><span class="text-lg">🔍</span><input v-model="searchKeyword" class="w-full bg-transparent text-sm font-semibold text-[var(--text)] outline-none placeholder:text-[var(--muted)]" :placeholder="$t('user_views.Home.search_placeholder')" /></div>
    </div>
    <div class="relative z-10 mb-8 flex flex-wrap gap-3">
      <button v-for="category in categoryTabs" :key="category.value" type="button" class="rounded-full border px-4 py-2 text-sm font-black transition hover:-translate-y-0.5 active:scale-95" :class="activeCategory === category.value ? 'border-[var(--border-strong)] bg-[var(--chip-active)] text-[var(--primary)] shadow-[0_12px_30px_rgba(155,44,255,0.18)]' : 'border-[var(--border)] bg-[var(--surface)] text-[var(--muted)] hover:border-[var(--border-strong)] hover:text-[var(--text)]'" @click="activeCategory = category.value">{{ category.label }}</button>
    </div>
...
const heroStats = computed(() => [
  { value: allQuizzes.value.length, label: t('user_views.Home.stats.public_quizzes') },
  { value: new Set(allQuizzes.value.map((quiz) => quiz.category)).size, label: t('user_views.Home.stats.categories') },
  { value: allQuizzes.value.reduce((sum, quiz) => sum + Number(quiz.attempts || 0), 0), label: t('user_views.Home.stats.attempts') },
])
const previewCards = computed(() => [
  { value: allQuizzes.value.length, label: t('user_views.Home.preview.public_quizzes') },
  { value: allQuizzes.value.reduce((sum, quiz) => sum + Number(quiz.questions || 0), 0), label: t('user_views.Home.preview.total_questions') },
  { value: allQuizzes.value.reduce((sum, quiz) => sum + Number(quiz.attempts || 0), 0), label: t('user_views.Home.preview.attempts') },
  { value: `${allQuizzes.value.length ? Math.round(allQuizzes.value.reduce((sum, quiz) => sum + Number(quiz.avgScore || 0), 0) / allQuizzes.value.length) : 0}%`, label: t('user_views.Home.preview.average_score') },
])
...
const categoryTabs = computed(() => [{ value: ALL_CATEGORY, label: t('user_views.Home.all_category') }, ...new Set(quizSections.value.map((section) => section.category)).values()].map((category) => typeof category === 'string' ? { value: category, label: category } : category))
const filteredQuizSections = computed(() => {
  const keyword = searchKeyword.value.trim().toLowerCase()
  return quizSections.value.filter((section) => activeCategory.value === ALL_CATEGORY || section.category === activeCategory.value).map((section) => {
...
const buildSections = (quizzes) => {
  const grouped = quizzes.reduce((acc, quiz) => {
    const category = quiz.category || t('user_views.Home.random_category')
  ...
  return Object.entries(grouped).map(([category, quizzes]) => ({
    ...
    title: category,
    description: t('user_views.Home.category_description', { category }),
    category,
    quizzes,
  }))
}
