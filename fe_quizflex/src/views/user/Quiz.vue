<template>
  <section class="grid gap-6 py-8 xl:grid-cols-[minmax(0,1fr)_360px]">
    <article
      class="relative overflow-hidden rounded-[2rem] border border-[var(--border)] bg-[var(--surface)] p-5 shadow-[var(--shadow-soft)] backdrop-blur-2xl sm:p-8"
    >
      <div
        class="pointer-events-none absolute -left-24 -top-24 h-80 w-80 rounded-full bg-[var(--primary)]/20 blur-3xl"
      ></div>
      <div
        class="pointer-events-none absolute -bottom-24 right-0 h-80 w-80 rounded-full bg-[var(--accent)]/10 blur-3xl"
      ></div>

      <div class="relative z-10">
        <div
          v-if="isLoading"
          class="rounded-2xl border border-[var(--border)] bg-[var(--surface-soft)] p-5 text-sm font-bold text-[var(--muted)]"
        >
          Đang chuẩn bị bài thi...
        </div>
        <div
          v-if="errorMessage"
          class="mb-6 rounded-2xl border border-rose-500/30 bg-rose-500/10 p-4 text-sm font-bold text-rose-300"
        >
          {{ errorMessage }}
        </div>
        ...
        <template v-if="currentQuestion.id">
          <div class="mb-7 flex flex-wrap items-center justify-between gap-4">
            <div>
              <div class="mb-4 flex flex-wrap items-center gap-2">
                <span
                  class="rounded-full border border-[var(--border-strong)] bg-[var(--chip-active)] px-4 py-2 text-xs font-black uppercase tracking-[0.18em] text-[var(--primary)]"
                  >Câu {{ currentIndex + 1 }} /
                  {{ quizQuestions.length || 1 }}</span
                >
                <span
                  class="rounded-full border border-[var(--border)] bg-[var(--surface-soft)] px-4 py-2 text-xs font-black text-[var(--muted)]"
                  >{{ quizMeta.category || "Quiz" }}</span
                >
                <span
                  class="rounded-full border border-[var(--border)] bg-[var(--surface-soft)] px-4 py-2 text-xs font-black text-[var(--muted)]"
                  >{{ quizMeta.difficulty || "Vừa" }}</span
                >
              </div>
              <h1
                class="max-w-4xl text-3xl font-black leading-tight tracking-[-0.055em] text-[var(--text)] sm:text-5xl"
              >
                <MathText :text="currentQuestion.question" />
              </h1>
              <p
                class="mt-4 max-w-2xl text-sm font-medium leading-7 text-[var(--muted)]"
              >
                Chọn đáp án rồi chuyển câu. Khi hết thời gian, hệ thống tự nộp
                bài.
              </p>
            </div>
            ...
            <div class="grid gap-3 text-center">
              <div
                class="relative grid h-24 w-24 place-items-center rounded-full border border-[var(--border)] bg-[var(--surface-soft)] shadow-[var(--shadow-card)]"
              >
                <div
                ...
                ></div>
                <div
                  class="relative z-10 grid h-16 w-16 place-items-center rounded-full bg-[var(--surface-strong)]"
                >
                  <span class="text-lg font-black text-[var(--text)]"
          ...
          <div
            class="mb-8 overflow-hidden rounded-full border border-[var(--border)] bg-[var(--surface-soft)] p-1"
          >
            <div
          ...
          <div class="grid gap-4">
            ...
            >
              <div
                ...
                class="absolute inset-0 bg-gradient-to-r from-[var(--primary)]/10 via-[var(--primary-2)]/10 to-[var(--accent)]/10"
              ></div>
              <div class="relative z-10 flex items-center gap-4">
                <span
                  class="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-[var(--primary)] to-[var(--primary-2)] text-sm font-black text-white shadow-[0_14px_30px_rgba(155,44,255,0.24)]"
                  >{{ answer.key }}</span
                >
                <span class="font-bold leading-7 text-[var(--text)]"
                  ><MathText :text="answer.text"
                /></span>
                <span
                  v-if="selectedAnswer === answer.key"
                  class="ml-auto grid h-8 w-8 shrink-0 place-items-center rounded-full bg-[var(--chip-active)] text-sm font-black text-[var(--primary)]"
                  >✓</span
                >
              </div>
            </button>
    ...
    <aside class="grid content-start gap-5">
      <article
        class="relative overflow-hidden rounded-[2rem] border border-[var(--border)] bg-[var(--surface)] p-6 shadow-[var(--shadow-card)] backdrop-blur-2xl"
      >
        <div class="relative z-10">
          <p
          ...
          <h2
            class="mt-2 text-2xl font-black tracking-[-0.05em] text-[var(--text)]"
          >
            Đếm ngược
          </h2>
          <div
            class="mt-5 rounded-[1.5rem] border border-[var(--border-strong)] bg-[var(--chip-active)] p-5 text-center"
          >
            <p
              class="text-xs font-black uppercase tracking-[0.22em] text-[var(--muted)]"
            >
              Còn lại
            </p>
            <div
              class="mt-2 text-4xl font-black tracking-[0.08em] text-[var(--text)]"
            >
          ...
          <div class="mt-5 grid grid-cols-2 gap-3">
            <div
              class="rounded-2xl border border-[var(--border)] bg-[var(--surface-soft)] p-4"
            >
              <p class="text-xs font-bold text-[var(--muted)]">Bài thi</p>
              <p class="mt-1 text-xl font-black text-[var(--text)]">
                {{ quizMeta.title || "Quiz" }}
              </p>
            </div>
            <div
      ...
      >
        ...
        <div class="mt-4 grid grid-cols-5 gap-2">
          <button
            ...
            :key="index"
            type="button"
            class="grid h-10 place-items-center rounded-2xl border text-xs font-black transition hover:-translate-y-0.5"
...
const progressRingStyle = computed(() => ({
  background: `conic-gradient(var(--accent-2) 0 ${progressPercent.value}%, var(--surface-soft) ${progressPercent.value}% 100%)`,
}));
...
const getAnswerClass = (key) =>
  selectedAnswer.value === key
    ? [
        ...
        "bg-[var(--chip-active)]",
        "shadow-[0_18px_44px_rgba(155,44,255,0.16)]",
      ]
    : [
        "border-[var(--border)]",
        "bg-[var(--surface-soft)]",
