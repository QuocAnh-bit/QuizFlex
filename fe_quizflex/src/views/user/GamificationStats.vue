...
| Badge icon
|--------------------------------------------------------------------------
| Không dùng emoji/icon từ backend nữa.
| Backend vẫn có thể trả badge.icon nhưng giao diện sẽ dùng icon library.
*/
...
const weekDays = computed(() => {
  ...
  return labels.map((label, i) => ({
    label,
      ...
      i < todayIndex ||
      (i === todayIndex && stats.value.current_streak > 0),
...
onMounted(async () => {
  try {
    ...
    console.error(
      'Lỗi khi tải thông tin thành tích:',
      error
    )
  }
})
