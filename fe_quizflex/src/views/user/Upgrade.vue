<template>
  <section class="max-w-[1200px] mx-auto py-6 space-y-8">
    ...
    <div class="card p-8 sm:p-12 text-center space-y-3">
      <span class="inline-flex items-center gap-1.5 rounded-full bg-purple-50 border border-purple-200 px-3.5 py-1 text-xs font-bold uppercase tracking-wider text-[#7C3AED]">
        ✦ Gói dịch vụ cao cấp
      </span>
      <h1 class="text-3xl sm:text-4xl lg:text-5xl font-black text-slate-900 tracking-tight">
        Mở khóa toàn bộ <span class="text-[#7C3AED]">đặc quyền học tập</span>
      </h1>
      <p class="text-sm sm:text-base text-slate-600 max-w-2xl mx-auto leading-relaxed">
        Nâng cấp để sử dụng AI sinh câu hỏi, quét tài liệu OCR không giới hạn và tạo phòng thi đấu thời gian thực cùng bạn bè.
      </p>
    </div>
    ...
    <div v-if="currentUser" class="space-y-4">
      ...
      >
        <div class="space-y-1">
          <span class="rounded-full bg-emerald-100 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-emerald-800">
            Trải nghiệm miễn phí
          </span>
          <h3 class="text-lg font-bold text-slate-900">Dùng thử Gói Plus Miễn Phí 7 Ngày</h3>
          <p class="text-xs text-slate-600 leading-relaxed max-w-2xl">
            Trải nghiệm đầy đủ tính năng tạo phòng thi đấu, phòng bài tập và nhận ngay <b>+20 lượt AI sinh đề</b>. Kích hoạt một lần duy nhất cho mỗi tài khoản.
          </p>
        </div>
        <button 
          @click="activateFreeTrial"
          :disabled="isActivatingTrial"
          class="btn-success text-xs shrink-0 px-5 py-2.5"
        >
      ...
      >
        <div class="space-y-1">
          <span class="rounded-full bg-purple-100 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-[#7C3AED]">
            Trạng thái dùng thử
          </span>
          ...
          <p class="text-xs text-slate-600">
            Đặc quyền dùng thử của bạn kết thúc vào: <b class="text-[#7C3AED]">{{ formatTrialExpiry(currentUser.vip_expires_at) }}</b>. Hết hạn sẽ tự động về gói Free.
          </p>
        </div>
        <span class="text-xs font-semibold text-slate-500 bg-white px-3 py-1.5 rounded-lg border border-slate-200">
    ...
    <div class="grid gap-6 md:grid-cols-3 items-stretch">
      <article 
        ...
        :class="plan.popular ? 'border-2 border-[#7C3AED] shadow-md relative' : 'border border-slate-200'"
      >
        <!-- Popular Badge -->
        <span 
          v-if="plan.popular" 
          class="absolute top-4 right-4 bg-[#7C3AED] text-white text-[10px] font-bold uppercase tracking-wider px-3 py-1 rounded-full"
        >
          Phổ biến nhất
        </span>
        ...
        <div class="space-y-4">
          ...
          <div class="pt-2">
            <template v-if="plan.id === 'plus_1m' && showPlusTrial">
              <span class="text-sm line-through text-slate-400 font-semibold mr-1.5">50.000đ</span>
              <span class="text-3xl font-black text-slate-900">0đ</span>
              <span class="text-xs text-slate-500 font-medium"> / 7 ngày</span>
            </template>
            <template v-else>
              <span class="text-3xl font-black text-slate-900">{{ plan.priceLabel }}</span>
              <span v-if="plan.price > 0" class="text-xs text-slate-500 font-medium"> / gói</span>
            </template>
          </div>

          <p class="text-xs text-slate-600 leading-relaxed border-b border-slate-100 pb-4">
            {{ plan.desc }}
          </p>
          ...
          <div class="space-y-2.5 pt-1">
            ...
            >
              <span class="text-[#7C3AED] font-bold mt-0.5">✓</span>
        ...
        <div class="pt-6 mt-4 border-t border-slate-100">
          <button 
            ...
            @click="activateFreeTrial"
            class="btn-success text-xs w-full py-2.5"
          >
          ...
          <template v-else>
            <button 
              ...
              disabled
              class="btn-secondary text-xs w-full py-2.5 cursor-not-allowed opacity-75"
            >
              ✓ Đang sử dụng
            </button>
            <button 
              ...
              disabled
              class="btn-secondary text-xs w-full py-2.5 cursor-not-allowed opacity-50"
            >
              🔒 Không thể hạ cấp
            </button>
            <button 
    ...
    >
      <div class="border-b border-slate-100 pb-3">
        <h3 class="text-base font-bold text-slate-900">Lịch sử giao dịch</h3>
        <p class="text-xs text-slate-500">Xem lại các hóa đơn và gói dịch vụ đã mua.</p>
      </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left text-xs">
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="item in historyList" :key="item.id" class="hover:bg-slate-50">
              <td class="py-3 pr-4 font-mono font-bold text-slate-800">{{ item.order_code }}</td>
              <td class="py-3 px-4 font-bold text-slate-900">{{ item.plan_name || getPlanNameByAmount(item.amount) }}</td>
              <td class="py-3 px-4">
                <span class="rounded bg-slate-100 px-2 py-0.5 font-bold uppercase text-[10px] text-slate-600">
                  {{ item.provider || 'Nội bộ' }}
                </span>
              </td>
              <td class="py-3 px-4 font-bold text-slate-900">{{ formatPrice(item.amount) }}</td>
              <td class="py-3 px-4">
                <span class="rounded-full px-2.5 py-0.5 text-[10px] font-bold" :class="getStatusBadgeClass(item.status)">
                  {{ getStatusText(item.status) }}
                </span>
              </td>
              <td class="py-3 pl-4 text-slate-500">{{ formatDate(item.created_at) }}</td>
            </tr>
    ...
    <Transition name="fade">
      ...
      >
        <div class="relative w-full max-w-[480px] rounded-2xl border border-slate-200 bg-white p-6 shadow-xl space-y-4">
          <!-- Close Button -->
          <button 
            @click="closeCheckout"
            class="absolute top-4 right-4 h-8 w-8 flex items-center justify-center rounded-lg border border-slate-200 bg-slate-50 text-slate-500 hover:bg-slate-100 text-xs"
          >
            ✕
          </button>
          ...
          <div class="text-center space-y-1">
            <span class="text-3xl block mb-1">{{ selectedPlan.icon }}</span>
            <h4 class="text-xl font-bold text-slate-900">Xác nhận nâng cấp</h4>
            <p class="text-xs text-slate-500">Vui lòng chọn phương thức thanh toán</p>
          </div>

          <!-- Order Summary Card -->
          <div class="rounded-xl border border-slate-100 bg-slate-50 p-4 space-y-2 text-xs">
            <div class="flex items-center justify-between font-semibold">
              <span class="text-slate-500">Gói cước:</span>
              <span class="text-slate-900 font-bold">{{ selectedPlan.name }}</span>
            </div>
            <div class="flex items-center justify-between font-semibold">
              <span class="text-slate-500">Thời hạn:</span>
              <span class="text-slate-900 font-bold">{{ selectedPlan.period }}</span>
            </div>
            <div class="flex items-center justify-between font-semibold">
              <span class="text-slate-500">AI Quota cộng thêm:</span>
              <span class="text-[#7C3AED] font-bold">+{{ selectedPlan.quota }} lượt</span>
            </div>
            
            <div v-if="selectedPlan.upgradeInfo && selectedPlan.upgradeInfo.unused_value > 0" class="pt-2 border-t border-slate-200 space-y-1">
              <div class="flex items-center justify-between text-slate-500">
                <span>Giá gốc gói mới:</span>
                <span class="line-through">{{ selectedPlan.priceLabel }}</span>
              </div>
            ...
            <div class="pt-2 border-t border-slate-200 flex items-center justify-between font-bold text-sm">
          ...
          <!-- Error Alert -->
          <div v-if="errorMessage" class="rounded-lg border border-red-200 bg-red-50 p-3 text-xs font-bold text-red-700">
            {{ errorMessage }}
          </div>
          ...
          <div class="space-y-2.5 pt-1">
            ...
            >
              <div class="flex items-center gap-3">
                <div class="h-9 w-9 shrink-0 rounded-lg bg-emerald-600 flex items-center justify-center text-white text-xs font-black">
                  QR
                </div>
                <div>
                  <b class="block text-xs font-bold text-slate-900">Thanh toán VietQR (PayOS)</b>
                  <span class="text-[11px] text-slate-500">Quét mã chuyển khoản từ mọi App Ngân hàng</span>
                </div>
              </div>
              <span class="text-xs font-bold text-emerald-700">Chọn →</span>
            </button>
            ...
            >
              <div class="flex items-center gap-3">
                <div class="h-9 w-9 shrink-0 rounded-lg bg-[#a21960] flex items-center justify-center text-white text-xs font-bold">
                  MoMo
                </div>
                <div>
                  <b class="block text-xs font-bold text-slate-900">Ví Điện Tử MoMo</b>
                  <span class="text-[11px] text-slate-500">Thanh toán tức thì qua ứng dụng MoMo</span>
                </div>
              </div>
              <span class="text-xs font-bold text-pink-700">Chọn →</span>
            </button>
          </div>

          <!-- Loading overlay -->
          <div v-if="isProcessing" class="absolute inset-0 bg-white/90 backdrop-blur-sm z-30 flex flex-col items-center justify-center gap-2 rounded-2xl">
            <div class="h-8 w-8 animate-spin rounded-full border-2 border-slate-200 border-t-[#7C3AED]"></div>
            <p class="text-xs font-bold text-slate-800">Đang kết nối cổng thanh toán...</p>
          </div>
        </div>
      </div>
    </Transition>
    ...
    <Transition name="fade">
      ...
      >
        <div class="relative w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-xl space-y-4">
          <div class="text-center space-y-1">
            <span class="text-3xl block">🎁</span>
            <h4 class="text-lg font-bold text-slate-900">Dùng Thử Plus 7 Ngày</h4>
            <p class="text-xs text-slate-500">Mở khóa đặc quyền gói Plus hoàn toàn miễn phí</p>
          </div>

          <div class="rounded-xl border border-slate-100 bg-slate-50 p-4 space-y-2 text-xs text-slate-700">
            ...
            <p class="pt-2 text-[11px] text-amber-700 font-semibold border-t border-slate-200">
              ⚠️ Mỗi tài khoản chỉ dùng thử 1 lần. Hết 7 ngày sẽ tự về gói Free mà không trừ tiền.
            </p>
          </div>

          <div v-if="trialErrorMessage" class="rounded-lg border border-red-200 bg-red-50 p-2.5 text-xs font-bold text-red-700">
            {{ trialErrorMessage }}
          </div>

          <div class="flex justify-end gap-2.5 pt-2">
            <button @click="closeConfirmTrialModal" :disabled="isActivatingTrial" class="btn-secondary text-xs">Hủy</button>
            <button @click="submitActivateTrial" :disabled="isActivatingTrial" class="btn-success text-xs">
    ...
    <Transition name="fade">
      ...
      >
        <div class="relative w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-xl text-center space-y-4">
          <div class="mx-auto h-14 w-14 flex items-center justify-center rounded-full bg-emerald-50 text-3xl">
            🎉
          </div>
          <div>
            <h4 class="text-lg font-bold text-slate-900">Kích hoạt thành công!</h4>
            <p class="text-xs text-slate-500">Tài khoản của bạn đã được nâng cấp lên gói Plus</p>
          </div>

          <div class="rounded-xl border border-slate-100 bg-slate-50 p-4 text-left text-xs space-y-2">
            <div class="flex justify-between">
              <span class="text-slate-500">Gói cước:</span>
              <span class="font-bold text-[#7C3AED]">Plus (7 ngày dùng thử)</span>
            </div>
            <div class="flex justify-between">
              <span class="text-slate-500">Lượt AI nhận thêm:</span>
              <span class="font-bold text-emerald-700">+20 lượt</span>
            </div>
            <div class="flex justify-between">
              <span class="text-slate-500">Thời hạn đến:</span>
              <span class="font-bold text-slate-900">{{ formatTrialExpiry(currentUser?.vip_expires_at) }}</span>
            </div>
          </div>

          <button @click="closeSuccessTrialModal" class="btn-primary text-xs w-full py-2.5">
