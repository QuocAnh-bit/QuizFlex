<template>
  <section class="py-4">
      ...
      <article class="card p-6 sm:p-8 space-y-6">
        ...
        >
          ...
          <button
            ...
            type="button"
            class="btn-primary text-xs px-4 py-2.5 flex items-center gap-1.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white shadow-sm cursor-pointer"
          ...
          >
            ...
            >
              <div class="flex items-center justify-between">
                ...
                <button
                  v-if="question.id && (!isOwner || quiz?.is_public)"
                  type="button"
    ...
    >
      <div class="card p-6 max-w-lg w-full space-y-4 shadow-2xl animate-scaleUp bg-white rounded-2xl">
        ...
        <div class="flex items-center justify-end gap-2.5 pt-3 border-t border-slate-100">
          ...
          <button
            type="button"
            class="btn-primary text-xs px-5 py-2 flex items-center gap-1.5 cursor-pointer"
            :disabled="isSubmittingReview"
...
const openQuestionReportModal = (question) => {
  const qObj = typeof question === 'object' ? question : { id: question }
  reportingQuestionId.value = qObj.id
  reportingQuestionSnippet.value = qObj.question || qObj.content || ''
  isQuestionReportModalOpen.value = true
}

const handleQuestionReported = () => {
  if (showToast) showToast('Đã gửi báo cáo câu hỏi thành công!', 'success')
}
