  ...
  <section class="mx-auto max-w-xl py-8">
    <article class="card p-6 sm:p-10 text-center space-y-6">
      <div class="space-y-1">
        ...
        <h1 class="text-2xl sm:text-3xl font-black text-slate-900 pt-2">Tham gia phòng bằng mã</h1>
        <p class="text-xs text-slate-600">Nhập mã phòng được cung cấp bởi giáo viên hoặc chủ phòng.</p>
      </div>

      <form class="rounded-2xl border border-slate-200 bg-slate-50 p-6 space-y-4" @submit.prevent="joinRoom">
        <input
      ...
      <div class="grid gap-3 text-left sm:grid-cols-2 text-xs">
        <div class="rounded-xl border border-slate-100 bg-slate-50 p-3.5 space-y-1">
          <p class="font-bold text-slate-800">📚 Nhận bài tập tự động</p>
          <p class="text-slate-500 leading-relaxed">Sau khi tham gia, danh sách các bài thi và hạn nộp sẽ hiển thị đầy đủ.</p>
        </div>
        <div class="rounded-xl border border-slate-100 bg-slate-50 p-3.5 space-y-1">
          <p class="font-bold text-slate-800">📊 Xem điểm & nhận xét</p>
          <p class="text-slate-500 leading-relaxed">Nhận phản hồi và điểm số từ chủ phòng sau khi nộp bài.</p>
        </div>
      </div>
