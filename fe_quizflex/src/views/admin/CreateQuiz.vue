    ...
    >
      ...
      <div class="relative z-10">
        ...
        <p class="mt-3 max-w-2xl text-sm leading-7 text-[var(--muted)]">
          Lưu quiz, câu hỏi, đáp án và ảnh bìa trực tiếp vào backend. Form này
          giờ đã có mặt mũi hơn một bảng nhập liệu hành chính.
        </p>
        ...
        <div class="mt-8 grid gap-5">
          ...
          >
            <div class="grid gap-0 lg:grid-cols-[minmax(0,1fr)_280px]">
              <div class="p-5">
                <div class="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    ...
                    <p class="mt-2 text-sm leading-6 text-[var(--muted)]">
                      Chỉ upload file ảnh từ máy. Nếu chưa chọn ảnh, hệ thống
                      dùng gradient dự phòng cho đỡ trống trải.
                    </p>
                  </div>
                  <button
                ...
                <div class="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_240px]">
                  ...
                  >
                    ...
                    <p
                      class="mt-1 text-xs font-bold leading-5 text-[var(--muted)]"
                    >
                      {{ selectedCoverLabel }}
                    </p>
                    <button
                  ...
                  <div class="grid gap-2 text-sm font-black text-[var(--text)]">
                    ...
                    >
                      ...
                      <div class="min-w-0">
                        <p
                          class="truncate text-sm font-black text-[var(--text)]"
                        >
                          {{ creatorProfile.name || "Người tạo hiện tại" }}
                        </p>
                        <p
                          class="truncate text-[11px] font-bold text-[var(--muted)]"
                        >
                          Avatar lấy từ hồ sơ.
                        </p>
                      </div>
              ...
              >
                <div
                  class="absolute inset-0 bg-gradient-to-t from-black/75 via-black/15 to-white/10"
                ></div>
                <div class="absolute bottom-4 left-4 right-4">
                  ...
                  <p
                    class="line-clamp-2 text-lg font-black leading-6 text-white drop-shadow"
                  >
        ...
        <div class="mt-8">
          <div class="mb-4 flex items-center justify-between gap-4">
            <h2
              class="text-2xl font-black tracking-[-0.05em] text-[var(--text)]"
            >
              Visibility
            </h2>
          ...
          <div class="grid gap-3 md:grid-cols-3">
            ...
            >
              ...
              <div class="relative z-10">
                ...
                <p
                  class="mt-2 text-xs font-semibold leading-5 text-[var(--muted)]"
                >
                  {{ option.description }}
                </p>
              </div>
        ...
        <div class="mt-8 grid gap-4">
          ...
          >
            <div class="mb-3 flex items-center justify-between gap-3">
              ...
              <button
                type="button"
                class="text-sm font-black text-[var(--danger)]"
            ...
            <label
              class="mt-3 grid gap-2 text-sm font-black text-[var(--text)]"
            >
              Đáp án đúng
    ...
    <aside class="grid content-start gap-5">
      ...
      >
        <div class="relative h-52" :style="{ background: coverBackground }">
          <div
            class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/10 to-white/10"
          ></div>
        ...
        <div class="p-6">
          ...
          <div class="mt-4 flex flex-wrap gap-2">
            <VisibilityBadge :value="form.visibility" />
            <span
              class="rounded-full border border-[var(--border)] bg-[var(--surface-soft)] px-3 py-1 text-xs font-black text-[var(--muted)]"
              >{{ difficultyText }}</span
            >
            <span
              class="rounded-full border border-[var(--border)] bg-[var(--surface-soft)] px-3 py-1 text-xs font-black text-[var(--muted)]"
              >{{ questions.length }} câu</span
            >
          </div>
        </div>
      </article>
...
const removeQuestion = (index) => {
  if (questions.value.length === 1) {
    errorMessage.value =
      "Quiz cần ít nhất 1 câu hỏi, xóa hết thì backend cũng chỉ biết thở dài.";
    return;
  }
...
const handleCoverFileChange = (event) => {
  ...
  if (file.size > 4 * 1024 * 1024) {
    errorMessage.value =
      "Ảnh bìa tối đa 4MB. Đừng bắt server nuốt cả album cưới.";
    event.target.value = "";
    return;
  }
...
const validateBeforeSave = () => {
  ...
  for (const [index, question] of questions.value.entries()) {
    ...
    if (!question.answers.some((answer) => answer.key === question.correct))
...
const syncCreatorProfile = (event) => {
  creatorProfile.value = event?.detail ??
    currentUserStorage.get() ?? { name: "Guest", email: "", avatar: "" };
};
