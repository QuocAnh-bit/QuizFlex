      ...
      <section class="w-full max-w-md rounded-2xl bg-white p-5 shadow-2xl">
        ...
        <div class="mt-5 space-y-3">
          <div class="rounded-xl border border-slate-200 p-4">
            <p class="text-xs font-black text-slate-800">Đặt cùng điểm cho tất cả câu</p>
            <div class="mt-3 flex items-center gap-2">
              <input v-model.number="samePoints" type="number" min="0.01" step="0.5" class="min-w-0 flex-1 rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-bold outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-100" />
              <span class="text-xs font-bold text-slate-500">điểm/câu</span>
              <button type="button" class="apply-button" :disabled="!validSame || !questionCount" @click="$emit('apply-same', samePoints)">Áp dụng</button>
            </div>
          </div>
...
const validSame = computed(() => Number.isFinite(Number(samePoints.value)) && Number(samePoints.value) > 0)
