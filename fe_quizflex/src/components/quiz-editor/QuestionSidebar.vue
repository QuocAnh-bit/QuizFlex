<template>
  <aside class="flex min-h-0 flex-col border-r border-slate-200 bg-white">
    ...
    <nav class="flex-1 space-y-2 overflow-y-auto p-3" aria-label="Danh sách câu hỏi">
      <button v-for="(question, index) in questions" :key="question.id" type="button" class="question-item group" :class="question.id === activeQuestionId ? 'question-item-active' : 'question-item-idle'" @click="$emit('select-question', question.id)">
        <span class="question-number" :class="question.id === activeQuestionId ? 'bg-violet-600 text-white' : 'bg-slate-100 text-slate-600'">{{ index + 1 }}</span>
...
const props = defineProps({ questions: { type: Array, default: () => [] }, activeQuestionId: { type: [Number, String], default: null }, questionOrigins: { type: Object, default: () => ({}) } })
defineEmits(['select-question', 'add-question'])
...
const sourceConfig = { personal: { label: 'Kho cá nhân', class: 'bg-violet-50 text-violet-700' }, bank: { label: 'Ngân hàng', class: 'bg-sky-50 text-sky-700' } }
...
.question-item-active { @apply border-violet-300 bg-violet-50 shadow-sm; }
.question-item-idle { @apply border-transparent bg-white hover:border-slate-200 hover:bg-slate-50; }
